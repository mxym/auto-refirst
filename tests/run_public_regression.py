...
def p0_model_and_pyc(binary:pathlib.Path,td:pathlib.Path) -> None:
    targets=["auto_refirst_public_model_trust_unit","auto_refirst_public_python_bytecode_unit","auto_refirst_public_flutter_codec_unit","auto_refirst_public_path_utf8_unit","auto_refirst_public_nuitka_unit"]
    ...
    log("[PASS P0] model-trust + generic-path/Nuitka synthetic units + direct CPython pyc trust ingress + Flutter codec")
...
def p1_generated(binary:pathlib.Path,td:pathlib.Path) -> None:
    ...
    env=dict(os.environ);env["PYTHONDONTWRITEBYTECODE"]="1"
    focused=(
        (".NET bundle/NativeAOT",[sys.executable,ROOT/"tests/test_dotnet_native_small.py","--cli",binary,"--tmp-root",td],240),
        ("Mach-O/Swift",[sys.executable,ROOT/"tests/test_macho.py",binary],240),
        ("Unreal Pak/IoStore",[sys.executable,ROOT/"tests/test_unreal_containers.py",binary],240),
        ("Hermes HBC",[sys.executable,ROOT/"tests/test_hermes.py",binary],360),
    )
    for label,argv,timeout in focused:
        cp=run(argv,env=env,timeout=timeout)
        assert "PASS" in cp.stdout,(label,cp.stdout)
        log(f"[PASS P1] {label} bounded source/project-generated static gate")
...
def sanitizer_smoke(harness:pathlib.Path,td:pathlib.Path) -> None:
    malformed={
      ...
      "bad-hermes.hbc":bytes.fromhex("c61fbc03c103191f")+b"\0"*7,
    }
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="auto-refirst-public-") as raw:
        ...
        if args.tier in ("P0","all"):
            ...
            p0_nested_and_graph(binary,td); p0_model_and_pyc(binary,td); p0_report_json(binary,td)
            p0_cli_exit_contract(binary); p0_windows_reparse(binary)

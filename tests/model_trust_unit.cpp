...
int main(){
    ...
    // original native execution succeeds.  The stronger native fact wins.
    {
        ...
        CHECK(a&&a->state==ModelTrustState::StaticModelContradictedByHigherEvidence);
        CHECK(a->evidence_ceiling==ModelEvidenceLevel::OriginalNativeResult);
        CHECK(r.guidance==ModelTrustState::StaticModelContradictedByHigherEvidence&&r.contradicted_model_scopes==1);
        CHECK(to_string(r.guidance)=="STATIC_MODEL_CONTRADICTED_BY_HIGHER_EVIDENCE");
    }
    ...
    // contradiction cannot overturn an exact reference.
    {
        ...
        CHECK(a&&a->state==ModelTrustState::StockModelSafe&&a->evidence_ceiling==ModelEvidenceLevel::ExactKnownGoodReference);
    }
    ...
    // Same-strength disagreement is preserved rather than arbitrarily won.
    {
        ...
        CHECK(a&&a->state==ModelTrustState::EvidenceConflictUnresolved&&r.evidence_conflict_scopes==1);
    }
    ...
    // Plane scoping: a match in one scope must not cover a mismatch in another.
    {
        ...
        CHECK(image&&image->state==ModelTrustState::StockModelSafe);
        CHECK(op&&op->state==ModelTrustState::UnresolvedModifiedRuntime);
        CHECK(r.guidance==ModelTrustState::UnresolvedModifiedRuntime&&r.stock_safe_scopes==1&&r.unresolved_modified_scopes==1);
    }
    ...
    // Explicit no-reference and pure model-only conclusions stay weak.
    {
        ...
        auto r=build_model_trust_report({unavailable});auto*a=find_model_trust_assessment(r,"runtime.semantic","stock");CHECK(a&&a->state==ModelTrustState::ReferenceUnavailable);
        ...
        r=build_model_trust_report({model});a=find_model_trust_assessment(r,"vm.semantic","stock-vm");CHECK(a&&a->state==ModelTrustState::ModelOnly&&r.guidance==ModelTrustState::ModelOnly);
    }
    ...
    // "concrete semantics recovered".  This is the Mixed-style contract.
    {
        ...
        auto f=model_trust_fact_from_exact_reference_delta(e);CHECK(f&&f->semantic_state==ModelSemanticState::ModifiedUnresolved&&f->reference_relation==ModelReferenceRelation::ExactDifference);
        auto r=build_model_trust_report({*f});auto*a=find_model_trust_assessment(r,e.semantic_scope,e.model_id);CHECK(a&&a->state==ModelTrustState::UnresolvedModifiedRuntime);
        e.exact_semantic_delta_recovered=true;e.delta_complete=false;e.delta_items=12;f=model_trust_fact_from_exact_reference_delta(e);CHECK(f&&f->semantic_state==ModelSemanticState::ModifiedUnresolved&&!f->scope_complete);
        r=build_model_trust_report({*f});a=find_model_trust_assessment(r,e.semantic_scope,e.model_id);CHECK(a&&a->state==ModelTrustState::UnresolvedModifiedRuntime);
        e.delta_complete=true;e.delta_items=37;f=model_trust_fact_from_exact_reference_delta(e);CHECK(f&&f->semantic_state==ModelSemanticState::ExactDeltaRecovered&&f->scope_complete&&f->delta_items==37);
        r=build_model_trust_report({*f});a=find_model_trust_assessment(r,e.semantic_scope,e.model_id);CHECK(a&&a->state==ModelTrustState::ExactModifiedSemantics);
    }
    ...
    // guidance remains conservative.
    {
        ...
        CHECK(op&&op->state==ModelTrustState::ExactModifiedSemantics);
        CHECK(r.guidance==ModelTrustState::Unresolved);
    }
    ...
    // Omitting model_id is allowed only when the semantic scope is unique.
    {
        ...
        auto r=build_model_trust_report(std::move(fs));
        CHECK(!find_model_trust_assessment(r,"shared.scope"));
        CHECK(find_model_trust_assessment(r,"shared.scope","model-a"));
    }
    ...
    // Filename/version/runtime-string bait cannot manufacture an exact delta.
    {
        ...
        CHECK(!model_trust_fact_from_exact_reference_delta(bait));
        bait.exact_reference_confirmed=true;CHECK(!model_trust_fact_from_exact_reference_delta(bait));
    }

...
class CappedSpoolStreambuf final:public std::streambuf {
    ...
    bool failed_=false;
    std::function<bool(std::uint64_t,std::uint64_t)> reserve_;
protected:
    std::streamsize xsputn(const char*s,std::streamsize n)override{
        ...
        if(written_<cap_){
            ...
            if(take>0){
                if(reserve_&&!reserve_(written_,static_cast<std::uint64_t>(take))){failed_=true;return 0;}
                out_.write(s,take);if(!out_){failed_=true;return 0;}written_=spool_sat_add(written_,static_cast<std::uint64_t>(take));
            }
        }
        return n;
    }
...
public:
    CappedSpoolStreambuf(std::ofstream&out,std::uint64_t cap,std::function<bool(std::uint64_t,std::uint64_t)> reserve={}):out_(out),cap_(cap),reserve_(std::move(reserve)){}
...
class DirectoryReportSpool {
    ...
    std::uint64_t resident_bytes_=0,cache_evicted_count_=0,reselected_count_=0;
    bool priorities_finalized_=false;
    ...
    void defer_record(std::size_t i,const char*reason){
        auto&r=records_[i];
    ...
    bool enforce_inline_budget(std::string&error){
        while(selected_bytes_>kDirectoryInlineReportBytes){
            ...
            defer_record(*worst,"DIRECTORY_INLINE_REPORT_BUDGET");
        }
        return true;
    }
    bool reserve_staging(std::uint64_t written,std::uint64_t next,std::string&error){
        const auto staging=spool_sat_add(written,next);
        if(staging>kDirectoryPerReportBytes){error="directory report staging invariant failed";return false;}
        while(resident_bytes_>kDirectoryReportSpoolHardBytes-staging){
            std::optional<std::size_t>worst;
            for(std::size_t i=0;i<records_.size();++i){
                const auto&r=records_[i];
                if(!r.selected&&!r.payload.empty()&&(!worst||record_better(records_[*worst],r)))worst=i;
            }
            if(!worst){error="directory report cache cannot free staging space";return false;}
            auto&r=records_[*worst];
            if(!remove_payload(r.payload,error))return false;
            resident_bytes_-=r.full_bytes;r.payload.clear();
            r.deferred_reason="DIRECTORY_SPOOL_CACHE_BUDGET";++cache_evicted_count_;
        }
        peak_bytes_=std::max(peak_bytes_,resident_bytes_+staging);
        return true;
    }
public:
    ...
    bool add(const std::filesystem::path&input,std::int64_t detail_priority,const std::function<void(std::ostream&)>&write_report,std::string&error){
        ...
        if(priorities_finalized_){error="directory report selection is already finalized";return false;}
        ...
        CappedSpoolStreambuf buf(out,kDirectoryPerReportBytes,[&](std::uint64_t written,std::uint64_t next){return reserve_staging(written,next,error);});
        ...
        peak_bytes_=std::max(peak_bytes_,spool_sat_add(resident_bytes_,staged_bytes));
    ...
    bool finalize_selection(const std::map<std::string,std::int64_t>&priorities,std::string&error){
        if(!ready()){error=error_;return false;}
        for(const auto&r:records_)if(!priorities.count(spool_path_key(r.input))){error="final directory report priority is missing";return false;}
        std::vector<std::size_t>available;
        for(std::size_t i=0;i<records_.size();++i){
            auto&r=records_[i];r.detail_priority=priorities.at(spool_path_key(r.input));
            if(!r.payload.empty())available.push_back(i);
        }
        std::sort(available.begin(),available.end(),[&](auto a,auto b){return record_better(records_[a],records_[b]);});
        selected_bytes_=0;reselected_count_=0;
        for(auto i:available){
            auto&r=records_[i];const bool selected=r.full_bytes<=kDirectoryInlineReportBytes-selected_bytes_;
            if(selected&&!r.selected)++reselected_count_;
            r.selected=selected;r.deferred_reason=selected?"":"DIRECTORY_INLINE_REPORT_BUDGET";
            if(selected)selected_bytes_+=r.full_bytes;
        }
        priorities_finalized_=true;
        return validate(error);
    }
    bool validate(std::string&error)const{
        if(!ready()){error=error_;return false;}
        std::uint64_t on_disk=0,inline_bytes=0;
        std::size_t resident_count=0;
        for(const auto&r:records_){
            if(r.selected&&r.payload.empty()){error="selected directory report has no payload";return false;}
        ...
        if(payload_count!=resident_count){error="unaccounted directory report spool payload";return false;}
        if(on_disk!=resident_bytes_||inline_bytes!=selected_bytes_||inline_bytes>kDirectoryInlineReportBytes||on_disk>kDirectoryReportSpoolHardBytes||peak_bytes_>kDirectoryReportSpoolHardBytes){error="internal directory report spool accounting invariant failed";return false;}
        return true;
    }

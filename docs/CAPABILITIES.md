...
- Unity pre-v108 ordinary MethodDef dispatch：metadata v106/v107 在 registration 唯一归一化到 106 或 106.1 后，可按版本匹配 runtime 的 CodeGenModule contract 恢复普通方法调度：MethodDef token 的 image-local RID 精确索引 `methodPointers[rid-1]` 与 `invokerIndices[rid-1]`，invoker `-1` 表示 missing-method，其他值必须落在全局 `CodeRegistration::invokerPointers`；`adjustorThunks` 必须按 MethodDef token 严格递增并绑定同 module owner；`Il2CppTokenAdjustorThunkPair` 的 ABI padding 不作为格式语义，只验证 token 与 thunk pointer。产品还要求 module `methodPointerCount` 与 image-local MethodDef 数量完全一致、token RID 无缺口/重复、method pointer 与独立 native-bound RVA 逐条一致，以及实际 method/invoker/adjustor pointer 均落在 executable image。该 `pre-v108-module-method+invoker+token-adjustor` profile 采用事务式提交：所有 module/image partition、method pointer、invoker 与 adjustor contract 全部闭合后才写回 MethodDef dispatch 字段，任一后段 module 失败都不会泄漏前序 module 的 partial invoker/adjustor evidence。该 profile 不声明 v108 才由 metadata 提供的 static-init type plane；v108 继续使用独立 `v108-image-invoker+token-adjustor+static-init` profile。
- Unity `Il2CppGenericClass` 在同一个 6000.6.0a6 / normalized-106.1 边界改变 native layout：旧 `context-32` 为 `type + context.class_inst + context.method_inst + cached_class`，其中 generic-class context 的 method-inst 必须为空；新 `class-inst-24` 为 `type + class_inst + cached_class`，不再存在 method-inst slot。normalized 106.1 与 v108 使用 24-byte profile，normalized 106 及既有 pre-106.1 versions 保持 32-byte profile；106/107 profile 尚未唯一闭合时不猜布局。compact profile 只读取 24 bytes，绝不读取 `+24` 邻接数据；`cached_class` 允许启动态 `0` 或 PE image pointer。JSON 保留历史 `tail_qword` 字段作为实际 `cached_class` 的兼容别名，并新增 `cached_class_va`、`generic_class_layout_profile`、`generic_class_record_size` 明确语义。

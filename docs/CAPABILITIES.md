...
- PyInstaller CArchive/PYZ，优先物化用户入口/高价值 Python bytecode 并进行子分析。PyInstaller bootstrap reference 按实际 loader generation 闭合：已验证的 4.10 使用旧 `pyimod01_os_path/02_archive/03_importers/04_ctypes` 四模块 profile；5.x/6.x 使用 `pyimod01_archive/02_importers/03_ctypes` 三个跨平台核心模块，只有 CArchive 实际包含 Windows-only `pyimod04_pywin32` 时才要求该模块同时匹配同一 reference generation。当前 compact reference 覆盖 Python 3.10 的 PyInstaller 4.10、5.13.2、6.0.0、6.16.0、6.22.2，以及 PyInstaller 6.22.2 在 Python 3.11/3.12 的跨平台 semantic reference，并保留既有 6.19/6.20 的 Python 3.13/3.14 reference；Python 3.10 的 modern Windows x64 reference 还包含真实官方-wheel onefile 中的 `pyimod04_pywin32`，因此 Windows profile 可在四个 required loader 模块共同匹配时闭合。对于同一 PyInstaller loader source 被不同 CPython 3.10 bugfix 编译器合法编译出的差异，semantic reference 可为同一 release label 保留多个经过真实样本验证的 hash；Windows 官方 3.10.0–3.10.11 矩阵确认 `pyimod02_importers` 只有三组稳定 compiler output（3.10.0、3.10.1–3.10.2、3.10.3–3.10.11），不会通过宽泛 control-flow normalization 合并未知差异。若多个发行版 loader payload 完全相同（例如当前 6.16.0/6.22.2 Python 3.10 三核心及 Windows `pyimod04_pywin32` raw payload），报告保留多版本 label 集合而不强猜具体发行版。

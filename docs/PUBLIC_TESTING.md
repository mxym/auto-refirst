...
- Unity GenericClass layout 外部/真实回放：版本匹配的 `js6pak/libil2cpp-archive` annotated tags `6000.5.0a6`（tag object `f2eb2039a28db6fdce01fdd030972b64b4fdbde2`，peeled commit `ddfa46701c5c4ac6b5418f36702d7dfbe045052f`）、`6000.6.0a6`（tag object `6648f63d92019efc39ba2622cee2527c38fc3fe6`，peeled commit `43698b1f03d2e13a0a7f6a68f2264a3d48e62b80`）与 `6000.6.0b1`（tag object `7d4ad897d11908f689d927bec23ec42008dc0bdb`，peeled commit `f28e4fbeb1006cfd3e5476d754b42d895208e6d8`）显示 `Il2CppGenericClass` 精确从 32-byte `type + GenericContext(class_inst,method_inst) + cached_class` 缩为 24-byte `type + class_inst + cached_class`，且 6000.6 runtime 创建 generic class 时先零初始化 `cached_class`、之后再原子发布。真实 Cpp2IL 6000.5/v106 的 9499 个 unique GenericClass 在 `+16/+24` 均为 0；真实 6000.6.0b1/v108 的 8337 个 unique structs 在真实 `+16 cached_class` 处 8337/8337 为 0，而旧 32-byte 假设读取的越界 `+24` 只有 2 个为 0、5385 个是小整数、2950 个是 image pointer，直接证明旧解析在读取邻接对象。新 profile 回放保持 v106 `11683/9499` 与 v108 `10292/8337` class/unique counts 不变，并分别报告 `context-32/32`、`class-inst-24/24`；两者实际 cached-class/tail compatibility counters 均为 0。受控 v108 mutation 把一个真实 cached slot 改为 PE image pointer时仍闭合（重复 registration refs 导致 2 个 logical rows观察同一 pointer），改成 `0x1234` 时 GenericClass plane 必须因 pointer 非 image 而 fail-closed。P0 profile unit 与 required sanitizer 锁定 106/106.1/107/v108 profile selection、record size、cached-class offset、method-inst presence；第三方 binary/source bytes 均不入库。
...
- 目录报告暂存独立 unit：核对当前/既有报告淘汰、重复超大报告延迟、真实磁盘条目与字节计账、写入失败及临时目录回收；

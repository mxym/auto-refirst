...
- Unity engine-version evidence 的 source-generated P0：严格 Unity 版本 grammar、serialized-file v21/v22 `globalgamemanagers` header、`UnityFS` v6-v8 header、有界向上目录关联、双来源 consensus/conflict，以及“仅在固定偏移出现版本字符串但 header 非法”不得确认；该证据当前只报告，不直接驱动 v106/v107 registration profile。

...
constexpr std::array<std::uint8_t,14> kBuildMagic={0xff,' ','G','o',' ','b','u','i','l','d','i','n','f',':'};

template<class T> bool read_le(std::span<const std::uint8_t>d,std::size_t o,T&v){if(o+sizeof(T)>d.size())return false;std::memcpy(&v,d.data()+o,sizeof(T));return true;}
...
std::optional<std::size_t> va_to_off(std::uint64_t va,const PeInfo&pe,const ElfInfo&elf,std::size_t fsize){
    ...
    if(elf.valid){
        for(const auto&s:elf.sections)if(va>=s.address&&va<s.address+s.size){auto delta=va-s.address;if(delta<s.size){auto o=s.offset+delta;if(o<fsize)return std::size_t(o);}}
        for(const auto&s:elf.segments)if(s.type==1&&va>=s.address){auto delta=va-s.address;if(delta<s.file_size&&s.offset+delta<fsize)return std::size_t(s.offset+delta);}
    }
    return{};
}
std::uint64_t off_to_va(std::size_t off,const PeInfo&pe,const ElfInfo&elf){
    if(pe.valid){if(off<pe.headers_size)return pe.image_base+off;for(const auto&s:pe.sections)if(off>=s.raw_offset&&off<std::uint64_t(s.raw_offset)+s.raw_size)return pe.image_base+s.rva+(off-s.raw_offset);}
    if(elf.valid){for(const auto&s:elf.sections)if(off>=s.offset&&off<s.offset+s.size)return s.address+(off-s.offset);for(const auto&s:elf.segments)if(s.type==1&&off>=s.offset&&off<s.offset+s.file_size)return s.address+(off-s.offset);}
    return 0;
}
std::uint64_t text_start(const PeInfo&pe,const ElfInfo&elf){
    if(pe.valid){for(const auto&s:pe.sections)if((s.characteristics&0x20000000)&&s.name==".text")return pe.image_base+s.rva;for(const auto&s:pe.sections)if(s.characteristics&0x20000000)return pe.image_base+s.rva;}
    if(elf.valid){for(const auto&s:elf.sections)if(s.name==".text"&&(s.flags&0x4))return s.address;for(const auto&s:elf.sections)if(s.flags&0x4)return s.address;for(const auto&s:elf.segments)if(s.type==1&&(s.flags&1)&&s.file_size)return s.address;}
    return 0;
}
bool executable_va(std::uint64_t va,const PeInfo&pe,const ElfInfo&elf){
    if(pe.valid){std::uint64_t rva=va>=pe.image_base?va-pe.image_base:va;for(const auto&s:pe.sections){auto n=std::max(s.vsize,s.raw_size);if((s.characteristics&0x20000000)&&rva>=s.rva&&rva<std::uint64_t(s.rva)+n)return true;}return false;}
    if(elf.valid){for(const auto&s:elf.segments)if(s.type==1&&(s.flags&1)&&va>=s.address&&va-s.address<s.memory_size)return true;}
    return false;
}
...
RTypeLite parse_rtype(std::span<const std::uint8_t>d,std::uint64_t va,const ModuleDataLite&m,const PeInfo&pe,const ElfInfo&elf,const GoInfo&g){RTypeLite r;r.va=va;auto o=va_to_off(va,pe,elf,d.size());if(!o)return r;auto ps=g.pointer_size;auto base=ps==8?48u:32u;if(base>d.size()-*o)return r;r.size=uptr(d,*o,ps,g.little_endian);r.tflag=d[*o+(ps==8?20:12)];r.kind=d[*o+(ps==8?23:15)];r.str_off=static_cast<std::int32_t>(u32(d,*o+(ps==8?40:24),g.little_endian));auto nv=static_cast<std::uint64_t>(static_cast<std::int64_t>(m.types)+r.str_off);auto nm=read_go_name(d,nv,pe,elf,g.version);if(!nm.valid)return r;r.name=nm.name;r.tag=nm.tag;if((r.tflag&2)&&!r.name.empty()&&r.name[0]=='*')r.name.erase(r.name.begin());auto kind=r.kind&0x1f;if(kind==0||kind>26||r.size>(1ull<<50))return r;r.valid=true;return r;}
bool candidate_types_ok(std::span<const std::uint8_t>d,const ModuleDataLite&m,const PeInfo&pe,const ElfInfo&elf,const GoInfo&g){auto tlo=va_to_off(m.typelinks,pe,elf,d.size());auto to=va_to_off(m.types,pe,elf,d.size());if(!tlo||!to||!m.typelinks_len||m.typelinks_len>10000000||m.typelinks_cap<m.typelinks_len)return false;if(m.typelinks_len*4>d.size()-*tlo)return false;std::size_t checked=0,good=0;for(std::size_t i=0;i<std::min<std::uint64_t>(m.typelinks_len,32);i++){auto va=read_signed32_as_addr(d,*tlo+i*4,g.little_endian,m.types);auto r=parse_rtype(d,va,m,pe,elf,g);++checked;if(r.valid&&!r.name.empty())++good;}return checked>=4&&good*100>=checked*70;}
ModuleDataLite find_moduledata(std::span<const std::uint8_t>d,const PeInfo&pe,const ElfInfo&elf,const GoInfo&g){ModuleDataLite best;auto ml=module_layout(g.version,g.pointer_size);if(!ml||!g.pclntab_va)return best;for(auto hit:find_ptr_occurrences(d,g.pclntab_va,g.pointer_size,g.little_endian)){auto need=std::max({ml->text,ml->types,ml->etypes,ml->typelinks+std::size_t(g.pointer_size)*3,ml->itablinks+std::size_t(g.pointer_size)*3});if(need>d.size()-hit)continue;ModuleDataLite m;m.layout=ml->name;m.va=off_to_va(hit,pe,elf);if(!m.va)continue;m.text=uptr(d,hit+ml->text,g.pointer_size,g.little_endian);m.types=uptr(d,hit+ml->types,g.pointer_size,g.little_endian);m.etypes=uptr(d,hit+ml->etypes,g.pointer_size,g.little_endian);m.typelinks=uptr(d,hit+ml->typelinks,g.pointer_size,g.little_endian);m.typelinks_len=uptr(d,hit+ml->typelinks+g.pointer_size,g.pointer_size,g.little_endian);m.typelinks_cap=uptr(d,hit+ml->typelinks+2*g.pointer_size,g.pointer_size,g.little_endian);m.itablinks=uptr(d,hit+ml->itablinks,g.pointer_size,g.little_endian);m.itablinks_len=uptr(d,hit+ml->itablinks+g.pointer_size,g.pointer_size,g.little_endian);m.itablinks_cap=uptr(d,hit+ml->itablinks+2*g.pointer_size,g.pointer_size,g.little_endian);if(!m.types||m.etypes<=m.types||m.etypes-m.types>(1ull<<31))continue;if(!va_to_off(m.types,pe,elf,d.size())||!va_to_off(m.typelinks,pe,elf,d.size()))continue;if(m.text&&text_start(pe,elf)&&m.text!=text_start(pe,elf)){auto delta=m.text>text_start(pe,elf)?m.text-text_start(pe,elf):text_start(pe,elf)-m.text;if(delta>(1ull<<20))continue;}if(!candidate_types_ok(d,m,pe,elf,g))continue;m.valid=true;return m;}return best;}
void parse_struct_fields(std::span<const std::uint8_t>d,GoTypeInfo&t,const RTypeLite&rt,const ModuleDataLite&m,const PeInfo&pe,const ElfInfo&elf,const GoInfo&g){if((rt.kind&0x1f)!=25)return;auto ps=g.pointer_size;std::uint64_t base=ps==8?48ull:32ull;auto slice_va=rt.va+base+ps;auto so=va_to_off(slice_va,pe,elf,d.size());if(!so||*so+3*ps>d.size())return;auto data=uptr(d,*so,ps,g.little_endian),len=uptr(d,*so+ps,ps,g.little_endian),cap=uptr(d,*so+2*ps,ps,g.little_endian);if(!data||len>10000||cap<len)return;auto fo=va_to_off(data,pe,elf,d.size());if(!fo||len*ps*3>d.size()-*fo)return;t.fields.reserve(std::size_t(len));for(std::uint64_t i=0;i<len;i++){auto p=*fo+std::size_t(i*ps*3);auto nameva=uptr(d,p,ps,g.little_endian),typeva=uptr(d,p+ps,ps,g.little_endian),oe=uptr(d,p+2*ps,ps,g.little_endian);auto nm=read_go_name(d,nameva,pe,elf,g.version);auto fr=parse_rtype(d,typeva,m,pe,elf,g);if(!nm.valid||!fr.valid)continue;GoStructField f;f.name=nm.name;f.tag=nm.tag;f.type_name=fr.name;f.type_va=typeva;f.offset=oe>>1;f.embedded=(oe&1)!=0;t.fields.push_back(std::move(f));}}

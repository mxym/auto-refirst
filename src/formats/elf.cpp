    ...
    // therefore fail the ordinary strict plane while still leaving exact raw loader facts here.
    {
        ...
        if(!out.elf64||!le||!(out.machine==62||out.machine==183)){
            ...
            if(!dyn_sources){im.state="NOT_PRESENT";}
                ...
                for(const auto&s:out.segments){if(s.type!=1||(!has_textrel&&!(s.flags&2))||!s.file_size||s.offset>=d.size())continue;const auto end=s.offset+std::min<std::uint64_t>(s.file_size,d.size()-s.offset);auto it=d.begin()+static_cast<std::ptrdiff_t>(s.offset),last=d.begin()+static_cast<std::ptrdiff_t>(end);while(it<last){it=std::search(it,last,elfmagic.begin(),elfmagic.end());if(it==last)break;const auto base=static_cast<std::uint64_t>(it-d.begin());inspect_embedded(base);it+=4;}}

    ...
    for(const auto&r:reports){
        ...
        if(r.elf_valid){
            if(!r.elf_interpreter.empty()){
                ...
                if(auto soname_it=elf_soname.find(interp_name);soname_it!=elf_soname.end()){matches=soname_it->second;soname_match=true;}else if(auto basename_it=elf_basename.find(interp_name);basename_it!=elf_basename.end())matches=basename_it->second;
            ...
            for(const auto&needed:r.elf_needed){if(needed.empty()||!emitted_names.insert(needed).second)continue;std::vector<std::size_t> matches;bool soname_match=false;
                if(auto soname_it=elf_soname.find(needed);soname_it!=elf_soname.end()){matches=soname_it->second;soname_match=true;}else if(auto basename_it=elf_basename.find(needed);basename_it!=elf_basename.end())matches=basename_it->second;

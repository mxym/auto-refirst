...
std::string render_text(const AnalysisReport& r) {
    ...
    if (r.golang.valid) {
        o << "Go runtime:\n"
          ...
          << "  pclntab: layout=" << r.golang.pclntab_layout << " file_off=0x" << std::hex << r.golang.pclntab_offset << " va=0x" << r.golang.pclntab_va << std::dec
          << " ptr=" << r.golang.pointer_size << " quantum=" << r.golang.quantum << "\n"
    ...
    for (const auto &cp : r.cpython_runtimes) {
        ...
        if(!cp.region_diffs.empty()){o<<"  native region differences:\n";for(const auto&r:cp.region_diffs)o<<"    "<<r.kind<<" "<<r.section<<" RVA=0x"<<std::hex<<r.rva<<std::dec<<" size="<<r.size<<" "<<r.detail<<"\n";}
...
void render_json_impl(std::ostream& o,const AnalysisReport& r,bool automatic_child_summary) {
    ...
    if(r.pyinstaller.valid){o<<",\"state\":\"CONFIRMED\",\"modified_cookie\":"<<(r.pyinstaller.heuristic_cookie?"true":"false")<<",\"cookie_offset\":"<<r.pyinstaller.cookie_offset<<",\"archive_start\":"<<r.pyinstaller.archive_start<<",\"archive_length\":"<<r.pyinstaller.archive_length<<",\"python_version\":"<<r.pyinstaller.python_version<<",\"python_library\":\""<<esc(r.pyinstaller.python_library)<<"\",\"entry_count\":"<<r.pyinstaller.entries.size();}
...

    {
        ...
        o << "  \"apk\": {\"candidate\":"<<(r.apk.candidate?"true":"false")<<",\"valid\":"<<(r.apk.valid?"true":"false")<<",\"state\":\""<<(r.apk.valid?"CONFIRMED":(r.apk.candidate?"FAILED":"ABSENT"))
          ...
          <<",\"invalid_dex_entry_count\":"<<r.apk.invalid_dex_entry_count<<",\"invalid_native_entry_count\":"<<r.apk.invalid_native_entry_count<<",\"anomaly_samples_truncated\":"<<(r.apk.anomaly_samples_truncated?"true":"false")

...
void render_unity_json(std::ostream& o,const UnityInfo& u,const UnityExtractResult& ex) {
    ...
    o<<"{\"valid\":"<<(u.valid?"true":"false")<<",\"il2cpp\":"<<(u.il2cpp?"true":"false")<<",\"mono\":"<<(u.mono?"true":"false")
     ...
     <<",\"engine_version_state\":\""<<esc(u.engine_version_state)<<"\",\"engine_version\":\""<<esc(u.engine_version)<<"\",\"engine_version_source\":\""<<esc(u.engine_version_source)<<"\",\"engine_version_detail\":\""<<esc(u.engine_version_detail)<<"\",\"engine_version_root\":\""<<esc(path_utf8(u.engine_version_root))<<"\""
     <<",\"globalgamemanagers_path\":\""<<esc(path_utf8(u.globalgamemanagers_path))<<"\",\"globalgamemanagers_state\":\""<<esc(u.globalgamemanagers_state)<<"\",\"globalgamemanagers_version\":\""<<esc(u.globalgamemanagers_version)<<"\",\"globalgamemanagers_detail\":\""<<esc(u.globalgamemanagers_detail)<<"\""
     <<",\"data_unity3d_path\":\""<<esc(path_utf8(u.data_unity3d_path))<<"\",\"data_unity3d_state\":\""<<esc(u.data_unity3d_state)<<"\",\"data_unity3d_version\":\""<<esc(u.data_unity3d_version)<<"\",\"data_unity3d_detail\":\""<<esc(u.data_unity3d_detail)<<"\""
     ...
     <<",\"metadata_registration_resolved\":"<<(u.metadata_registration_resolved?"true":"false")<<",\"metadata_registration_va\":"<<u.metadata_registration_va<<",\"metadata_registration_profile_state\":\""<<esc(u.metadata_registration_profile_state)<<"\",\"metadata_registration_profile\":\""<<esc(u.metadata_registration_profile)<<"\",\"metadata_registration_normalized_variant\":\""<<esc(u.metadata_registration_normalized_variant)<<"\",\"metadata_registration_engine_hint\":\""<<esc(u.metadata_registration_engine_hint)<<"\",\"metadata_registration_profile_detail\":\""<<esc(u.metadata_registration_profile_detail)<<"\",\"metadata_registration_error\":\""<<esc(u.metadata_registration_error)<<"\""
...
std::string render_text(const AnalysisReport& r) {
    ...
    if (r.pyinstaller.valid) {
        ...
        if(!r.pyinstaller.bootstrap_reference_status.empty()){
            ...
            if(!r.pyinstaller.bootstrap_reference_generation.empty())o<<" core_generation="<<r.pyinstaller.bootstrap_reference_generation.substr(0,16);
            if(!r.pyinstaller.bootstrap_windows_extension_generation.empty())o<<" windows_generation="<<r.pyinstaller.bootstrap_windows_extension_generation.substr(0,16);
            if(!r.pyinstaller.bootstrap_release_candidates.empty())o<<" release_candidates="<<r.pyinstaller.bootstrap_release_candidates;
            if(!r.pyinstaller.bootstrap_reference_label.empty())o<<" direct_reference="<<r.pyinstaller.bootstrap_reference_label;
            if(!r.pyinstaller.bootstrap_match_mode.empty())o<<" mode="<<r.pyinstaller.bootstrap_match_mode;
            o<<"\n";
            for(const auto&m:r.pyinstaller.bootstrap_modules){o<<"    "<<m.name<<": "<<m.state;if(!m.reference_label.empty())o<<" ref="<<m.reference_label;if(!m.reference_component.empty())o<<" component="<<m.reference_component;if(!m.reference_generation.empty())o<<" generation="<<m.reference_generation.substr(0,16);if(!m.semantic_error.empty())o<<" error=+0x"<<std::hex<<m.semantic_error_offset<<std::dec<<" "<<m.semantic_error;o<<"\n";}
        }
    ...
    if (r.unity.valid || r.unity.metadata_valid) {
        ...
        o<<"  metadata_version="<<u.metadata_version<<" layout="<<u.metadata_layout<<"\n"
         <<"  Engine version: state="<<u.engine_version_state<<" version="<<u.engine_version<<" source="<<u.engine_version_source<<" root="<<path_utf8(u.engine_version_root)<<"\n"
         ...
         <<"  MetadataRegistration: resolved="<<(u.metadata_registration_resolved?"true":"false")<<" VA=0x"<<std::hex<<u.metadata_registration_va<<std::dec<<" profile_state="<<u.metadata_registration_profile_state<<" profile="<<u.metadata_registration_profile<<" normalized="<<u.metadata_registration_normalized_variant<<" engine_hint="<<u.metadata_registration_engine_hint<<" registered_types="<<(u.registered_types_resolved?"validated":"failed")<<':'<<u.registered_type_count<<" field_offsets="<<u.field_offset_type_count<<" runtime_only="<<u.field_offset_runtime_only_type_count<<" type_sizes="<<u.type_size_type_count<<"\n"
        ...
        if(!u.metadata_registration_profile_detail.empty()&&u.metadata_registration_profile_state=="AMBIGUOUS")o<<"  MetadataRegistration profile note: "<<u.metadata_registration_profile_detail<<"\n";
...
void render_json_impl(std::ostream& o,const AnalysisReport& r,bool automatic_child_summary) {
    ...
    if(r.pyinstaller.valid){
        o<<",\"state\":\"CONFIRMED\",\"modified_cookie\":"<<(r.pyinstaller.heuristic_cookie?"true":"false")<<",\"cookie_offset\":"<<r.pyinstaller.cookie_offset<<",\"archive_start\":"<<r.pyinstaller.archive_start<<",\"archive_length\":"<<r.pyinstaller.archive_length<<",\"python_version\":"<<r.pyinstaller.python_version<<",\"python_library\":\""<<esc(r.pyinstaller.python_library)<<"\",\"entry_count\":"<<r.pyinstaller.entries.size()
         <<",\"bootstrap_profile\":\""<<esc(r.pyinstaller.bootstrap_profile)<<"\",\"bootstrap_reference_status\":\""<<esc(r.pyinstaller.bootstrap_reference_status)<<"\",\"bootstrap_reference_label\":\""<<esc(r.pyinstaller.bootstrap_reference_label)<<"\",\"bootstrap_reference_generation\":\""<<esc(r.pyinstaller.bootstrap_reference_generation)<<"\",\"bootstrap_windows_extension_generation\":\""<<esc(r.pyinstaller.bootstrap_windows_extension_generation)<<"\",\"bootstrap_release_candidates\":\""<<esc(r.pyinstaller.bootstrap_release_candidates)<<"\",\"bootstrap_source_equal_releases\":\""<<esc(r.pyinstaller.bootstrap_source_equal_releases)<<"\",\"bootstrap_match_mode\":\""<<esc(r.pyinstaller.bootstrap_match_mode)<<"\",\"bootstrap_modules\":[";
        for(std::size_t i=0;i<r.pyinstaller.bootstrap_modules.size();++i){if(i)o<<',';const auto&m=r.pyinstaller.bootstrap_modules[i];o<<"{\"name\":\""<<esc(m.name)<<"\",\"state\":\""<<esc(m.state)<<"\",\"reference_available\":"<<(m.reference_available?"true":"false")<<",\"reference_label\":\""<<esc(m.reference_label)<<"\",\"reference_component\":\""<<esc(m.reference_component)<<"\",\"reference_generation\":\""<<esc(m.reference_generation)<<"\",\"size\":"<<m.size<<",\"sha256\":\""<<esc(m.sha256)<<"\",\"semantic_sha256\":\""<<esc(m.semantic_sha256)<<"\",\"normalized_semantic_sha256\":\""<<esc(m.normalized_semantic_sha256)<<"\",\"normalized_code_units\":"<<m.normalized_code_units<<",\"normalization_source\":\""<<esc(m.normalization_source)<<"\",\"semantic_error_offset\":"<<m.semantic_error_offset<<",\"semantic_error\":\""<<esc(m.semantic_error)<<"\",\"normalize_error\":\""<<esc(m.normalize_error)<<"\"}";}o<<"]";
    }

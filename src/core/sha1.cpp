...
struct Sha1Stream {
    ...
    std::array<std::uint8_t,20> finish(){auto bits=bytes*8;buf[used++]=0x80;if(used>56){while(used<64)buf[used++]=0;block(h,buf.data());used=0;}while(used<56)buf[used++]=0;for(int i=7;i>=0;--i)buf[used++]=std::uint8_t(bits>>(i*8));block(h,buf.data());std::array<std::uint8_t,20>o{};for(int i=0;i<5;i++){o[i*4]=static_cast<std::uint8_t>((h[i]>>24)&0xffu);o[i*4+1]=static_cast<std::uint8_t>((h[i]>>16)&0xffu);o[i*4+2]=static_cast<std::uint8_t>((h[i]>>8)&0xffu);o[i*4+3]=static_cast<std::uint8_t>(h[i]&0xffu);}return o;}
};
}

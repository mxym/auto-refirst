...
NuitkaInfo detect_nuitka(std::span<const std::uint8_t>d,const PeInfo&pe,const ElfInfo&elf,std::uint64_t max_decompressed_size){
    ...
    // not project any outer-loader tuple onto the payload. The extracted child
    // can be analyzed independently and recover its own tuple.
    if(best.valid&&!best.onefile&&!best.constant_blocks.empty())best.compiled_version=detect_nuitka_compiled_version(d,elf);

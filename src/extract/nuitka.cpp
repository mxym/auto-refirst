...
constexpr std::size_t kOnefileFilenameChars=1024;
// Nuitka's onefile bootstrap has used a fixed filename_char_t[1024] payload
// filename buffer across the researched 0.6.19.7 -> 4.2 generations. Mirror
// that runtime geometry here instead of accepting arbitrarily long printable
// data after an incidental KA[X/Y] marker. The terminating NUL consumes one
// slot, so at most 1023 filename characters are structurally admissible.
bool safe_name(std::string_view n){if(n.empty()||n.size()>=kOnefileFilenameChars)return false;if(n.find("..")!=std::string_view::npos)return false;std::size_t printable=0;for(unsigned char c:n){if(c==0)return false;if(c>=0x20&&c<0x7f)++printable;}return printable*100/n.size()>80;}
std::optional<std::pair<std::string,std::size_t>> read_name8(std::span<const std::uint8_t>d,std::size_t p){auto q=p;while(q<d.size()&&q-p<kOnefileFilenameChars&&d[q])++q;if(q>=d.size()||q-p>=kOnefileFilenameChars)return{};std::string n(reinterpret_cast<const char*>(d.data()+p),q-p);return std::pair{n,q+1};}
std::optional<std::pair<std::string,std::size_t>> read_name16(std::span<const std::uint8_t>d,std::size_t p){std::string n;for(std::size_t q=p;q+1<d.size()&&n.size()<kOnefileFilenameChars;q+=2){auto c=std::uint16_t(d[q])|(std::uint16_t(d[q+1])<<8);if(!c)return std::pair{n,q+2};if(c>0x7f)return{};n.push_back(char(c));}return{};}

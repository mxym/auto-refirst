...
void compare_text_regions(std::span<const std::uint8_t>d,const PeInfo&pe,const CPythonReference&struct_ref,CPythonInfo&out){
    ...
    auto meaningful=std::min<std::uint64_t>(it->vsize,it->raw_size);out.text_chunks_reference=static_cast<std::uint32_t>(tr->count);out.text_chunks_target=static_cast<std::uint32_t>((meaningful+kCPythonTextBlockSize-1)/kCPythonTextBlockSize);
    for(std::size_t i=0;i<tr->count;i++){auto rel=std::uint64_t(i)*kCPythonTextBlockSize;auto ref_len=std::min<std::uint64_t>(kCPythonTextBlockSize,tr->text_meaningful>rel?tr->text_meaningful-rel:0);if(!ref_len)break;if(rel>=meaningful){push_region(out.region_diffs,"MISSING_OFFICIAL_TEXT",".text",it->rva+static_cast<std::uint32_t>(rel),ref_len,"reference .text bytes absent from target");continue;}auto len=std::min<std::uint64_t>(ref_len,meaningful-rel);auto off=std::uint64_t(it->raw_offset)+rel;if(off+len>d.size()){push_region(out.region_diffs,"MISSING_OFFICIAL_TEXT",".text",it->rva+static_cast<std::uint32_t>(rel),len,"target .text raw bytes unavailable");continue;}auto h=fnv64(d.subspan(static_cast<std::size_t>(off),static_cast<std::size_t>(len)));if(len==ref_len&&h==tr->hashes[i])++out.text_chunks_matched;else push_region(out.region_diffs,"PATCHED_TEXT_CHUNK",".text",it->rva+static_cast<std::uint32_t>(rel),len,"same-relative .text chunk differs from official reference");}
...
CPythonInfo detect_cpython(std::span<const std::uint8_t>d,const PeInfo&pe,const std::string&source){
    ...
    for(const auto&s:pe.sections)all.insert(s.name);
    for(const auto&kv:rs)all.insert(kv.first);
